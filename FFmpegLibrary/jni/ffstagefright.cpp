...
#define LOG_LEVEL 15
#define LOG_TAG2 "stagefright.c"
#define LOGI(level, ...) if (level <= LOG_LEVEL) {__android_log_print(ANDROID_LOG_INFO, LOG_TAG2, __VA_ARGS__);}
#define LOGE(level, ...) if (level <= LOG_LEVEL) {__android_log_print(ANDROID_LOG_ERROR, LOG_TAG2, __VA_ARGS__);}
    ...
    virtual status_t read(
        ...
        if (ret == OK) {
            ...
            if (ret == OK) {
                ...
                memcpy((uint8_t *)(*buffer)->data() + (*buffer)->range_offset(), frame->buffer, frame->size);
...
void* decode_thread(void *arg)
{
    ...
    do {
        ...
        if (frame->status == OK) {
            ...
            const unsigned char *ptr = (unsigned char *)buffer->data() + buffer->range_offset();
            if (av_image_fill_arrays(src_data, src_linesize,
                                            ptr, avctx->pix_fmt, w, h, 1) < 0) {
            	LOGE(1, "Could not fill image");
            }
...
static av_cold int Stagefright_init(AVCodecContext *avctx)
{
    ...
    if (!s->bsfc) {
    	LOGE(1, "Cannot open the h264_mp4toannexb BSF!");
